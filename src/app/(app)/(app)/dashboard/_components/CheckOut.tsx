  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const { error, paymentIntent } = await stripe.confirmPayment({
      ...
      confirmParams: {
        ...
        }/payments/success?amount=${amount}&flightId=${flightId}&seatType=${
          seatType || ""
        }&seatCount=${seatCount || 1}&paymentIntentId=${
    ...
    if (error) {
      ...
      try {
        ...
        if (bookingResponse.ok) {
          ...
          window.location.href = `${process.env.NEXT_PUBLIC_BASE_URL}/payments/success?amount=${amount}`;
        } else {
